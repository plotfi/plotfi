# Puyan Lotfi - AI/ML Compiler Engineer

## 📫 Contact
* Discord: @puyanlotfi on [GPUMODE](https://discord.gg/gpumode)
* Email: `plotfi [at] gmail [dot] com`

Can't find me anywhere else...

## µ-Resume:

### Meta (2018–Present)
- GPU/ML compiler development with Triton DSL → Currently developing Triton's Extension system
- Swift C++ interop → Swift 5.9 / Xcode 15
- Clang Interface Stubs / llvm-ifs (adopted by [Google Fuchsia](https://fuchsia.googlesource.com/fuchsia/+/5e786c3))
- Objective-C Optimizations → Selector compression (ie `hello:world:` to `a::`), `objc_direct` ABI (ie `_<a:b:c:>`)

### Apple (2013–2018)
- LLVM GPU compiler backend (A8–A12, iOS 8–12)
- Built Apple's first LLVM-based GPU assembler (A11/A12)
- GPU binary tooling, disassembler used in simulators and architectural experiments
- Metal Shader Profiler (Xcode 10) — [Patent](https://patents.google.com/patent/US10310830B2)

### Intel (2011–2013)
- C for Metal GPU compiler; authored CM GPU printf

## Education
- MS/BS Computer Science — Georgia Tech

## Talks
- [Clang Interface Stubs](https://bit.ly/35ap4uk) — 2019 LLVM Dev Meeting
- [MIR-Canon](https://bit.ly/39P3ikG) — 2018 EuroLLVM ([video](https://youtu.be/RHT-bh_xo6U))
